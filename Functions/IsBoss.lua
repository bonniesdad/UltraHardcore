...
-- Function to check if a unit is a dungeon or raid boss
function IsBoss(unit)
    ...
    -- Check for dungeon boss using NPC ID from GUID
    -- Capture the 5th numeric field (npcID) before the final hex segment
    local npcID = tonumber(unitGUID:match("^Creature%-%d+%-%d+%-%d+%-%d+%-(%d+)%-[%x]+$"))
    if not npcID then return false, false end

    local isDungeonBoss = DungeonBossIDs and DungeonBossIDs[npcID] or false
    local isRaidBoss    = RaidBossIDs[npcID] or false

    return isDungeonBoss, isRaidBoss
end

-- Example usage with slash command for testing
SLASH_CHECKBOSS1 = "/checkboss"
SlashCmdList["CHECKBOSS"] = function()
    local isDungeonBoss, isRaidBoss = IsBoss("target")
    local unitName = UnitName("target") or "Unknown"
    if isDungeonBoss then
        print(unitName .. " is a dungeon boss!")
    elseif isRaidBoss then
        print(unitName .. " is a raid boss!")
    else
        print(unitName .. " is not a boss.")
    end
end