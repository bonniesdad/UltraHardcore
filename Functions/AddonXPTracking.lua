...
local msgPrefix =
  yellowTextColour .. '[|r' .. redTextColour .. 'ULTRA|r' .. yellowTextColour .. ']|r '
...
function AddonXPTracking:ShouldRecalculateXPGainedWithAddon()
  ...
  elseif stats.xpGWA == 0 and stats.xpGWOA == 0 and stats.xpTotal > 0 then
    self:XPTrackingDebug(
      'Recalculating because xpGWA and xpGWOA are both 0, xpTotal is ' .. stats.xpTotal
    )
    return true
  else
...
function AddonXPTracking:ForceSave()
  ...
  AddonXPTracking:XPTrackingDebug(
    'Setting XP values: ' .. totalXP .. ' - ' .. stats.xpGWA .. ' = ' .. stats.xpGWOA
  )
end
...
function AddonXPTracking:NewLastXPUpdate(levelUp, currentTime)
  ...
  if levelUp then
    ...
    self:XPTrackingDebug(
      'Falsifying last update timestamp from ' .. currentTime .. ' to ' .. newTime
    )
    return newTime
  else
    return currentTime
...
SlashCmdList['XPFORLEVEL'] = function(msg)
  ...
  print(
    'A level ' .. greenTextColour .. msg .. '|r character has at least ' .. redTextColour .. formatNumberWithCommas(
      totalXP
    ) .. '|r XP'
  )
end
